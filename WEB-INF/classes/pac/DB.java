...
public class DB {
  ...
  private DB() throws Exception {
	Class.forName("org.postgresql.Driver");
  	...
  	// Clients:
        ps_select_client = cnx.prepareStatement ("select nom, adr from pac_client where ncli=?");
        ps_select_clients = cnx.prepareStatement ("select ncli,nom, adr from pac_client order by ncli");
  		ps_insert_client = cnx.prepareStatement ("insert into pac_client values(?,?,?)");
        ps_update_client = cnx.prepareStatement ("update pac_client set nom=?,  adr=? where ncli=?");
        ps_delete_client = cnx.prepareStatement ("delete from pac_client where ncli=?");

	// achats:
  	ps_select_achatsClient = cnx.prepareStatement ("select ncli,np,qa from pac_achat where ncli=? order by np");
  	ps_select_achatsClientCP = cnx.prepareStatement ("select a.ncli,a.np,qa,nom,lib from pac_achat a, pac_client c,pac_produit p where p.np=a.np and c.ncli=a.ncli and a.ncli=? order by a.np");
  	ps_select_achatsProduit = cnx.prepareStatement ("select ncli,np,qa from pac_achat where np=? order by ncli");
  	ps_select_achatsProduitCP = cnx.prepareStatement ("select a.ncli,a.np,qa,nom,lib from pac_achat a, pac_client c,pac_produit p where p.np=a.np and c.ncli=a.ncli and a.np=? order by a.np");

	// produits:
  	ps_select_pdts = cnx.prepareStatement ("select np,lib,coul,qs from pac_produit order by np");
  	ps_select_pdt = cnx.prepareStatement ("select np,lib,coul,qs from pac_produit where np=?");
        ps_update_pdt = cnx.prepareStatement ("update pac_produit set qs=? where np=?");
